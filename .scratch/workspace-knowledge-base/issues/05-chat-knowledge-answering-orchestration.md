...
把工作區知識庫接進現有聊天流程，讓 `Knowledge Answering` 可以依 workspace 預設值啟用，並允許使用者只對單一 **Turn** 暫時覆寫；同時由較高層的 turn orchestration 決定是否做 retrieval、如何組查詢、何時 fallback 成一般聊天，以及如何維持 `ChatService` 作為底層單純的 LLM streaming adapter。
